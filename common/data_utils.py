...
from .camera import world_to_camera, normalize_screen_coordinates, add_noise_me
...
def create_2d_data(data_path, dataset):
    keypoints = np.load(data_path, allow_pickle=True) # 加载存储二维关键点数据的文件 允许加载包含 Python 对象的文件
    keypoints = keypoints['positions_2d'].item() # 提取加载数据中的 'positions_2d' 键对应的字典
    ...
    for subject in keypoints.keys():
        for action in keypoints[subject]:
            for cam_idx, kps in enumerate(keypoints[subject][action]):
                ...
                # 改 直接在原始数据上加噪
                # kps[...,1:3] = add_noise_me(kps[..., 1:3],x_error,y_error)
