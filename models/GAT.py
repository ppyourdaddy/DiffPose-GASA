...
class GATLayer(nn.Module):
    # 最后一层 input_feature=192 output_feature=96 
    def __init__(self,input_feature,output_feature,dropout,alpha,concat=True):
        super(GATLayer,self).__init__()
        self.input_feature = int(input_feature)
        self.output_feature = int(output_feature)
        self.alpha = alpha
        self.dropout = dropout
        self.concat = concat
        self.a = nn.Parameter(torch.empty(size=(2*int(output_feature),1)))
        self.w = nn.Parameter(torch.empty(size=(int(input_feature),int(output_feature))))
    ...
    def forward(self,h,adj):
        # Wh = torch.mm(h,self.w) #h:[1,17,192] w:[192,24]
        Wh = torch.bmm(h, self.w.unsqueeze(0).expand(h.size(0), -1, -1))
        ...
        h_prime = torch.bmm(attention, Wh) # 得到下一层的输入 # a:[1,17,17] wh:[1,17,24]
