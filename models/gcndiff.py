...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    """
    assert len(timesteps.shape) == 1# 确保输入一维张量

    half_dim = embedding_dim // 2 # 计算嵌入向量维度的一半 正弦和余弦两部分
...
class GCNdiff(nn.Module):
    def __init__(self, adj, config):
        ...
        gcn = GraphNet2(adj=adj, in_features=dim_model, out_features=dim_model, n_pts=n_pts)
    ...
    def forward(self, x, mask, t, cemd):
        ...
        temb = nonlinearity(temb)#sigmoid
